  ...
  || (navigator.platform === "MacIntel" && typeof navigator.standalone !== "undefined"));
	...
	resizeCanvas:       function ()
	{
		...
		if (scaleX != this.scaleX || scaleY != this.scaleY)
		{
			...
			var objParent = this.canvas.offsetParent;
			if (objParent) {
			    objParent.style.width = this.canvas.width.toString() + "px";
			    objParent.style.height = this.canvas.height.toString() + "px";
		...
		if (this.playApplication(false))
		{
			...
			else
			{
				...
				if (this.dataToLoad.size() == 0 && this.dataLoading.size() == 0)
					bComplete = true;
                if (this.preloader != null && (((this.dwOptions & CRunApp.AH2OPT_LOADDATAATSTART) != 0 && !this.preloader.wasExecuted) || (this.frame.html5Options & CRunFrame.HTML5FOPT_DISPLAYPRELOADER) != 0))
				{
					if (!this.resetted)
					{
                    ...
                    if (bComplete)
                        this.preloader.wasExecuted = true;
				}
				if (bComplete)
				{
					this.resetted = false;
	...
	endApplication: function ()
	{
		...
		// Stop sounds == n
		if (this.soundPlayer != null && !this.isPreloaderSubApp)
		{
    ...
    // Enable sounds in Chrome, actual 292.27 all browser need this, IE was superseed by EDGE
	enableSoundsInChrome: function ()
	{
	    if (!this.browserDetect.isIE && this.soundContext != null && this.soundContext.state == 'suspended')
...
CRunFrame.prototype =
{
	loadFullFrame: function (index)
	{
		...
		if (this.app.dwOptions & CRunApp.AH2OPT_LOADDATAATSTART)
		{
			...
			if (this.app.mosaicMaxHandle == 0 || this.app.isPreloaderSubApp)
...
CSoundPlayer.prototype =
{
		...
		if (this.dataToDecode != null && this.dataToDecode.size() > 0)
		{
			if (!this.decoding)
			{
				...
				that.context["decodeAudioData"](cSound.response, function (buffer)
				{
      			...
      			function(e)
      			{ 
      				console.log("Sound Error, name: "+cSound.name+ " and error: " + e.message);
      			});
			}
		}
	},
	...
	play: function (handle, nLoops, channel, bPrio, _volume, _freq)
	{
		var n;
		...
		if (channel < 0)
        {
            ...
            {
                // No free channel found, take the first unlocked, interruptible one
				for (n = 0; n < CSoundPlayer.NCHANNELS; n++)
				{
        ...
        if (_volume != -1)
            this.volumes[channel] = _volume;
        sound.play(nLoops, bPrio, this.volumes[channel]);
        if (_freq != 0)
            sound.setFrequency(_freq);
	},
    ...
    getSndNameChannel: function (channel)
	{
		if (channel >= 0 && channel < CSoundPlayer.NCHANNELS)
		{
            if (this.channels[channel] != null && (this.channels[channel].isPlaying() || this.channels[channel].isPaused()))
			{
...
CPreloaderFrame.prototype =
{
	...
	isComplete: function ()
	{
		...
		if (bComplete)
		{
			...
			var quit = this.subApp.run.doRunLoop()
			if(quit !== 0)
				this.subApp.context.clearBackground(0, 0, this.subApp.canvas.width, this.subApp.canvas.height);
