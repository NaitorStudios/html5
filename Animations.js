	...
	animIn: function (vbl)
	{
		...
		if (frame == 0)
		{
			...
			if ((this.hoPtr.hoAdRunHeader.rhFrame.leFlags & CRunFrame.LEF_TIMEDMVTS) != 0) {
			    // Calculate the delta as a float to preserve precision
			    let newDelta = aDelta * this.hoPtr.hoAdRunHeader.rh4MvtTimerCoef;

			    // Separate the fractional part and add it to the accumulator
			    this.hoPtr.roc.fractionalDelta += newDelta - Math.floor(newDelta);

			    // If the accumulated fractional part is greater than or equal to 1, add it to the integer part
 			   if (this.hoPtr.roc.fractionalDelta >= 1.0) {
			        aDelta = Math.floor(newDelta) + Math.floor(this.hoPtr.roc.fractionalDelta);
			        this.hoPtr.roc.fractionalDelta -= Math.floor(this.hoPtr.roc.fractionalDelta); // Subtract the whole number part from the accumulator
			    } else {
        			// Otherwise, just use the integer part
        			aDelta = Math.floor(newDelta);
			    }
			}
